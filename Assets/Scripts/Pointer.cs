...
/// </example>
public class Pointer : VRTK_WorldPointer
{
    ...
    protected override void Update()
    {
        ...
        if (pointer.gameObject.activeSelf)
        {
            ...
            HandleEvents(pointerCollidedWith);
        }
    }

    private void HandleEvents(RaycastHit pointerCollidedWith)
    {
        PointerResponder previousPointerResponder = null;
        if (_selectedObject != null)
        {
            previousPointerResponder = _selectedObject.GetComponent<PointerResponder>();
        }

        if (pointerCollidedWith.collider == null)
        {
            _selectedObject = null;
            if (previousPointerResponder != null)
            {
                previousPointerResponder.OnExit();
            }
            return;
        }
        
        var currentObject = pointerCollidedWith.collider.gameObject;

        var currentPointerResponder = currentObject.GetComponent<PointerResponder>();

        if (currentPointerResponder != null && currentObject != _selectedObject)
        {
            currentPointerResponder.OnEnter();
        }

        if (currentObject != _selectedObject && previousPointerResponder != null)
        {
            previousPointerResponder.OnExit();
        }

        _selectedObject = pointerCollidedWith.collider.gameObject;
    }

    void OnClick(object sender, ControllerInteractionEventArgs args)
    {
        if (_selectedObject == null) return;

        var pointerResponder = _selectedObject.GetComponent<PointerResponder>();

        if (pointerResponder != null)
        {
            pointerResponder.OnClick();
        }
    }
